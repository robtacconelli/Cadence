"""MTA subway hourly ridership per station: SECOND uncontaminated test of the 'aggregate human-demand'
hypothesis. EIA-930, 50 US stations, hourly, Jan-Jun 2026 -- well
...
SETS=sorted(os.path.basename(f)[:-4] for f in glob.glob("data/transit/*.npy"))
XF={n:np.load(f"data/transit/{n}.npy").astype(np.float64) for n in SETS}
SETS=[n for n in SETS if len(XF[n])>=START+N+H]
print(f"{len(SETS)} station series of length {len(XF[SETS[0]]) if SETS else 0}",flush=True)
...
if len(sys.argv)>1 and sys.argv[1]=="gpu":
  ...
  np.savez(f"{TD}/transitq_N{N}.npz",
...
d=np.load(f"{TD}/transitq_N{N}.npz"); res={}
print(f"MTA SUBWAY RIDERSHIP (MTA subway hourly, 2026, {len(SETS)} stations)")
...
json.dump(res,open("results/transit_bench.json","w"),indent=1,default=float)
